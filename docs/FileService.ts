  ...
  private async initializeGoogleAuth() {
    try {
      const credentials = {
        ...
        project_id: process.env.ENV_GOOGLE_PROJECT_ID,
        private_key_id: process.env.ENV_GOOGLE_PRIVATE_KEY_ID,
        private_key: process.env.ENV_GOOGLE_PRIVATE_KEY?.replace(/\\n/g, "\n"),
        client_email: process.env.ENV_GOOGLE_CLIENT_EMAIL,
        client_id: process.env.ENV_GOOGLE_CLIENT_ID,
        auth_uri: "https://accounts.google.com/o/oauth2/auth",
        token_uri: "https://oauth2.googleapis.com/token",
        auth_provider_x509_cert_url:
          "https://www.googleapis.com/oauth2/v1/certs",
        client_x509_cert_url: `https://www.googleapis.com/robot/v1/metadata/x509/${process.env.ENV_GOOGLE_CLIENT_EMAIL}`,
      };
    ...
    switch (type) {
      case 'audio':
        ...
        docs = (await Promise.all(transcriptions.map(async (transcription, index) => {
          const metadata = {
            ...
            chunk_index: index,
            total_chunks: transcriptions.length,
      ...
      case 'text':
        ...
        if (chunkSize) {
          ...
          const chunks = await this.textService.split(textContent, chunkSize, baseMetadata);
          ...
          docs = [await this.textService.document(textContent, undefined, { 
            source: originalPath, 
            path: storagePath, 
            name: basename(originalPath), 
            mimeType, 
      ...
      case 'image':
        ...
        docs = [await this.textService.document(
          ...
          undefined,
          { source: originalPath, path: storagePath, name: basename(originalPath), mimeType, source_uuid: fileUUID, uuid: uuidv4() }
        )];
        break;
      default:
