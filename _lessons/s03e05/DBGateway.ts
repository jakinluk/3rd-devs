...
class DBGateway {
    ...
    async query<T>(sql: string): Promise<DBResponse<T>> {
        const payload: Payload = {
            task: this.params.task,
            apikey: this.params.apiKey,
            query: sql
        };
        return await this.httpClient.post<Payload, DBResponse<T>>(this.params.endpoint, payload, "json");
    }
}



export { type Payload as SubmitPayload, DBGateway, type DBResponse };